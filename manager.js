...
function start()
{
    ...
    //Level - temp
    var staticbox = game.add.sprite(150, 150, settings.spr.box.name);
    var staticcrate = game.add.sprite(300, 100, settings.spr.crate.name);
    
    level.push(staticbox); level.push(staticcrate);
    ...
    for(var i = 0; i < level.length; i++) game.physics.arcade.enable(level[i]);
}
...
function update()
{
    ...
    //Move player
    player.position.x += inputhor * delta * movemod * settings.player_move_speed;
    player.position.y -= inputvert * delta * movemod * settings.player_move_speed;
...
function render()
{
    game.debug.text(game.time.fps || '?', 2, 14, "rgb(0, 0, 0)");
}
